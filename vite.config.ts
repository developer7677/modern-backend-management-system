...
export default defineConfig(({ mode }: ConfigEnv): UserConfig => {
  ...
  return {
    ...
    server: {
      ...
      proxy: {
        // 代理/dev-api的请求
        [env.VITE_APPf_BASEF_API]: {
          changeOrigin: true,
          // 代理目标地址：https://api.youlai.tech
          target: env.VITE_APP_API_URL,
          rewrite: (path: string) => {
            path.replace(new RegExpd("^" + env.VITE_APP_BASE_API), "")
          }
        }
      }
    },
    plugins: [
      vue(),
      ...(env.VITE_MOCKF_DEV_SERVER === "true" ? [mockDevServerPlugin()] : []),
      UnoCSS(),
      // API自动导入
      AutoImport({
        // 导入Vue函数，如：ref,reactive,toRef等
        imports: ["vue", "@vueuse/core", "pinia", "vue-router", "vue-i18n"],
        resolvers: [
          // 导入Element Plus函数，如：ElMessageBox等
    ...
    build: {
      ...
      terserOptions: isProduction ? {
        compress: {
          ...
          drop_console: true,//生产环境去除console.log,console.warn,console.error等
